...
SITE_KEY='6LfadvIoAAAAAG4YiNApxRxQ48FGEGM4HxRUktLN'
SECRET_KEY='6LfadvIoAAAAAO5VM-BLdcK38hEu4xGUJrKZbbiV'
...
# Verify reCAPTCHA response and return True if valid, else return False
def runCaptcha()->bool:
    #return False
...
# Pass reCAPTCHA site key to login.html template for rendering
@app.route("/")
def index():
   print("login")
...
# Authenticate login credentials and handle reCAPTCHA verification failure
...
def login_auth():
    ...
    if not(runCaptcha()):
        message = f"Captcha Verification Failed"
...
# Process employee data insertion, validate login, and handle reCAPTCHA verification failure
...
def update_bug():
    ...
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        cursor.execute('SELECT * FROM bug where is_deleted=False')
...
def delete_bug(id_data):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("UPDATE bug SET is_deleted = 1 WHERE bug_id = %s", (id_data,))
...
#Query the database with is_deleted flag set to 1#
#Retrieve the deleted bugs
#Pass the list of result to the template page to view to the user
@app.route('/recover_bugs', methods=['GET'])
def recover_bugs():
    username = session['username']
    userlevel =session['user_level']
    ...
    field_values={
        'is_deleted': 1 #Selects the bugs that have been deleted. 
    }
    ...
    with connection.cursor() as cursor:
       ...
       # Pass the query result to the template to list the bugs 
        return render_template('recover_bugs_result.html',result=search_result)
    
    
#Function that queries the database and sets 
#is Deleted = False for the recovered bug 
#After recovering, redirect to update_bugs toute 
@app.route('/recover_bug', methods=['POST'])
def recover_bug():
    if "loggedin" not in session:
         message = f"You need to Login first"
         flash(message=message)
    ...
    if request.method == "POST":
        ...
        connection = pymysql.connect(**db_config)
        with connection.cursor() as cursor:
            #Modify the flag of the deleted bug to 0 
            cursor.execute("UPDATE bug SET is_deleted=False WHERE bug_id=%s",bug_id )
            connection.commit()
            
            message = f"Bug with id {bug_id} was successfully recovered."
...
def manage_employee():
    ...
    if session['user_level'] != 3:
        condition = False
        return render_template('dashboard.html', condition = condition, username=username, userlevel=userlevel,site_key=SITE_KEY)
    ...
    with connection.cursor() as cursor:
        ...
        return render_template('manage_employee.html', employees=data, username=username, userlevel=userlevel,site_key=SITE_KEY)
